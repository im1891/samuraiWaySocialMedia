...
export const profileAPI = {
	getUserProfile: (userId: number) => {
		return axiosInstance
			...
			.then((response: AxiosResponse<GetUserProfileResponseType>) => response.data)
	},
	getStatus: (userId: number) => {
		return axiosInstance.get<string>(`profile/status/${userId}`).then((res: AxiosResponse<string>) => res.data)
	},
...
export const authAPI = {
	me: () => {
		return axiosInstance
			...
			.then((response: AxiosResponse<ResponseType<UserDataType>>) => response.data)
	},
	...
	logout: () => {
		return axiosInstance.delete('auth/login').then((res: AxiosResponse<ResponseType>) => res.data)
	}
}
